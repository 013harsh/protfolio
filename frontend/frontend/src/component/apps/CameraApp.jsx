...
const TitleBar = memo(({ isMaximized, onMinimize, onMaximize, onClose, dragControls }) => (
  <div 
  ...
  >
    ...
    <div className="flex items-center gap-2">
      <button onClick={onMinimize} className="p-1 px-3 text-white/40 hover:bg-white/5 rounded-md"><Minus size={14} /></button>
      <button onClick={onMaximize} className="p-1 px-3 text-white/40 hover:bg-white/5 rounded-md">
        {isMaximized ? <Minimize2 size={14} /> : <Maximize2 size={14} />}
      </button>
      <button onClick={onClose} className="p-1 px-3 text-white bg-red-500/80 hover:bg-red-500 rounded-md"><X size={14} /></button>
    </div>
  </div>
));
...
const CameraApp = ({ windowId }) => {
  ...
  return (
    <motion.div
      ...
      dragMomentum={false}
      dragListener={false}
      initial={{ scale: 0.95, opacity: 0, x: 200, y: 100 }}
      animate={{
        scale: 1,
        opacity: 1,
      ...
      className={`absolute z-50 pointer-events-auto bg-black shadow-2xl border border-white/10 overflow-hidden flex flex-col ${isMaximized ? "rounded-none" : "rounded-xl"}`}
    >
      <TitleBar 
      ...
      <div className="relative flex-1 bg-black overflow-hidden flex items-center justify-center">
        ...
        <CameraControls 
          ...
          onSwitchCamera={() => {}} // Placeholder for now
        />
      </div>
    </motion.div>
  );
